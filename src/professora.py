...
# ==========================
def verificar_professor_aluno(aluno_id, professor_id):
    """Verifica se o aluno pertence ao professor."""
    if not aluno_id or not professor_id:
        return False
    res = supabase.table('alunos').select('professor_id').eq('id', aluno_id).execute()
    if not res.data:
        return False
    return res.data[0].get('professor_id') == professor_id

def verificar_professor(professor_id_rota, professor_id_token):
    """Verifica se o ID da rota coincide com o do token."""
...
def login_professor():
    ...
    # REMOVIDO o fallback para senha em texto puro
    senha_valida = check_password_hash(senha_banco, str(senha))
    
    if not senha_valida:
        return jsonify({"erro": "E-mail ou senha incorretos."}), 401

    # Gera token com os dados corretos
...
def lista_alunos(professor_id):
    # Valida se o professor da rota é o mesmo do token
...
def cadastrar_e_avaliar_aluno():
    try:
        ...
        # CORRIGIDO: vírgula entre 'cpf_aluno' e 'ano_escolar'
        ...
        # Valida se o professor_id da requisição é o mesmo do token
        ...
        # Opcional: verificar se CPF já existe (evitar duplicidade)
        # (pode ser adicionado)
        ...
        if not req_avaliacao.data:
            # Se falhar, podemos reverter a inserção do aluno? Idealmente usar transação.
...
def obter_desempenho_aluno(aluno_id):
    try:
        if not verificar_professor_aluno(aluno_id, request.professor_id):
            return jsonify({"erro": "Acesso não autorizado a este aluno."}), 403
